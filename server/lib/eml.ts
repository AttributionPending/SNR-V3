...
export function buildHtmlBody(opts: {
  ...
  const footerLine = opts.footerText?.trim() ||
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#eef0f3" style="background-color:#eef0f3;min-width:100%;">
        ...
        <td bgcolor="${esc(tlpColor)}" style="background-color:${esc(tlpColor)};padding:8px 24px;">
          <table width="100%" border="0" cellpadding="0" cellspacing="0">
            <tr>
              ...
              <td align="right">
                <span style="color:rgba(255,255,255,0.75);font-family:Arial,Helvetica,sans-serif;font-size:10px;letter-spacing:1.5px;text-transform:uppercase;">Security Intelligence Brief</span>
              </td>
            </tr>
          </table>
        </td>
          ...
          <table width="100%" border="0" cellpadding="0" cellspacing="0">
            <tr>
              <td valign="top">${opts.logoDataUri ? `
                <img src="${esc(opts.logoDataUri)}" alt="Logo" width="240" height="80" style="width:240px;height:auto;max-height:80px;max-width:240px;display:block;margin-bottom:12px;object-fit:contain;" />` : ''}
  ...
  // Apply configured font family / body size. Defaults preserve the original
  // template byte-for-byte when no customization is set.
  return applyFontSettings(htmlOut, opts.fontFamily, opts.bodyFontSize);
}
...
function applyFontSettings(html: string, fontFamily?: string, bodyFontSize?: string): string {
  let out = html;
  const stack = fontFamily ? EMAIL_FONT_STACKS[fontFamily] : undefined;
  if (stack && stack !== 'Arial,Helvetica,sans-serif') {
    out = out.split('font-family:Arial,Helvetica,sans-serif').join(`font-family:${stack}`);
  }
  const size = bodyFontSize ? parseInt(bodyFontSize, 10) : NaN;
