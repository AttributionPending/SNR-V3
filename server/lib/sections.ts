...
/**
 * Section keys populated deterministically server-side (NOT by the LLM).
 * They keep a normal type (e.g. 'text') for rendering, but are excluded from the
 * Phase 2 schema + guidance so the model never generates or hallucinates them.
 * `references` is built from the analyst's provided input — see lib/references.ts.
 */
export const DETERMINISTIC_KEYS = new Set<string>(['references']);
...
export const DEFAULT_SECTIONS: BriefSection[] = [
  ...
  },
  {
    ...
    label: 'References',
    type: 'text',
    description: 'Auto-populated from the provided input — URLs, CVE IDs, and Reference:/Source: lines (indicator/IOC URLs are excluded). Not generated by the AI.',
    enabled: true,
  },
  {
...
export function buildEmailSchema(sections: BriefSection[]): {
  ...
  for (const section of sections) {
    ...
    if (DETERMINISTIC_KEYS.has(section.key)) continue; // populated server-side, not by the LLM
...
export function buildSectionGuidance(sections: BriefSection[]): string {
  const textSections = sections.filter(s => s.enabled && !AUTO_TYPES.has(s.type) && !DETERMINISTIC_KEYS.has(s.key));
