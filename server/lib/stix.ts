...
/** One technique's aggregated detection coverage, as returned by /api/detections/coverage. */
...
const COVERAGE_COLORS: Record<CoverageTechnique['status'], string> = {
  covered: '#8ec843',   // rules written, no reported gap
  partial: '#ffd166',   // rules exist but analysis still reports a gap
  gap: '#ff6b6b',       // no rule and a reported gap
  unknown: '#6b7280',
};

/**
 * ATT&CK Navigator layer for aggregate detection coverage across all analysed
 * incidents — the cross-session counterpart to buildNavigatorLayer (which covers
 * a single incident). Scored 0–100 so Navigator's gradient also works if an
 * operator switches off the explicit colours.
 */
export function buildCoverageNavigatorLayer(
  techniques: CoverageTechnique[],
  teamLabel = 'All incidents',
): Record<string, unknown> {
  const score: Record<CoverageTechnique['status'], number> = { covered: 100, partial: 60, gap: 10, unknown: 0 };

  return {
    name: `SNR detection coverage: ${teamLabel}`,
    versions: { attack: '16', navigator: '5.1', layer: '4.5' },
    domain: 'enterprise-attack',
    description: `Aggregate detection coverage generated by Signal-to-Noise (SNR) — ${new Date().toISOString()}`,
    ...
    techniques: techniques.map((t) => ({
      ...
      ...(t.tactic ? { tactic: t.tactic.toLowerCase().replace(/\s+/g, '-') } : {}),
      score: score[t.status],
      color: COVERAGE_COLORS[t.status],
      comment:
        `Status: ${t.status}\nRules: ${t.rule_count}\n`
        + `Seen in ${t.sessions} incident(s) — ${t.detected_votes} likely detected, ${t.gap_votes} gap`,
      enabled: true,
      metadata: [
        { name: 'status', value: t.status },
        { name: 'rule_count', value: String(t.rule_count) },
        { name: 'sessions', value: String(t.sessions) },
        { name: 'gap_votes', value: String(t.gap_votes) },
      ],
      links: [],
      showSubtechniques: t.technique_id.includes('.'),
    })),
    gradient: { colors: ['#ff6666', '#ffe766', '#8ec843'], minValue: 0, maxValue: 100 },
    legendItems: [
      { label: 'Covered — rule written, no reported gap', color: COVERAGE_COLORS.covered },
      { label: 'Partial — rule exists but a gap is still reported', color: COVERAGE_COLORS.partial },
      { label: 'Gap — no rule, gap reported', color: COVERAGE_COLORS.gap },
      { label: 'Unknown', color: COVERAGE_COLORS.unknown },
    ],
    metadata: [
      { name: 'generated_by', value: 'SNR v1.0' },
      { name: 'generated_at', value: new Date().toISOString() },
